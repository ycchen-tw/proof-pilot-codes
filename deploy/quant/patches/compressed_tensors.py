...
# ===================== proof-pilot patch (dense MXFP4) =====================
# sglang has no dense FP4 weight-only scheme: a group-wise float4 weight-only
# checkpoint (MXFP4 = e2m1 weights + e8m0 group scale) matches _is_wNa16, hits
# the int4 WNA16 branch, fails the "pack-quantized" format check and raises
# "CompressedTensorsW4A16Sparse24 is not supported". This adds a dense MXFP4
# weight-only scheme. Routed from _get_scheme_from_parts for weight.type==float,
# num_bits==4, strategy==group (mxfp4/mxfp4a16). NVFP4 (strategy=tensor_group,
# +global scale) is untouched and still uses W4A4Fp4.
#
# Two execution paths (env PP_MXFP4_KERNEL):
#   "marlin"  (default): repack to Marlin + GEMM via float4_e2m1f weight type
#     (same kernel gpt-oss MXFP4 MoE uses on Hopper) -> 4-bit HBM traffic, decode
#     speed on par with int4 WNA16. Reuses sglang's gptq_marlin_repack /
#     marlin_permute_scales / mxfp4_marlin_process_scales / apply_gptq_marlin_linear.
#   "dequant" (fallback): e2m1+e8m0 -> bf16 at load, plain F.linear (bf16 speed,
#     no memory win). Used automatically if Marlin prep raises.
import os as _pp_os

import torch.nn.functional as _pp_F
from torch.nn.parameter import Parameter as _PPParameter

from sglang.srt.layers.parameter import (
    GroupQuantScaleParameter as _PPGroupScale,
    ModelWeightParameter as _PPModelWeight,
)

_PP_E2M1 = torch.tensor(
    [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0], dtype=torch.float32
)


def _pp_unpack_fp4(packed: torch.Tensor, m: int, n: int) -> torch.Tensor:
    """Unpack uint8 [m, n//2] (two e2m1 nibbles/byte) -> float32 [m, n].
    low nibble first, then high nibble (matches compressed_tensors)."""
    a = packed.flatten()
    high = (a & 0xF0) >> 4
    low = a & 0x0F
    combined = torch.stack((low, high), dim=1).flatten()
    signs = (combined & 0x08).bool()
    mag = (combined & 0x07).long()
    lut = _PP_E2M1.to(packed.device)
    vals = lut[mag] * torch.where(signs, -1.0, 1.0)
    return vals.reshape(m, n)


class CompressedTensorsW4A16Fp4Dense(CompressedTensorsLinearScheme):
    """Dense MXFP4 (e2m1 + e8m0 group scale) weight-only: Marlin fp4 (default)
    or dequant-to-bf16 fallback."""

    def __init__(self, group_size):
        self.group_size = int(group_size) if group_size else 32
        # default "dequant": the dense Marlin float4 kernel rejects group_size=32
        # for every layer shape on sm90 (thread-config check fails -- group32
        # scales don't fit the 228KB smem budget), and the failed validation
        # GEMMs can poison cuda-graph capture. Marlin is opt-in (works only where
        # the kernel finds a tiling, e.g. Blackwell / a future sglang).
        self.kernel = _pp_os.environ.get("PP_MXFP4_KERNEL", "dequant").lower()
        self._mode = None
    ...
    def create_weights(
        ...
        **kwargs,
        ...
        layer.output_size_per_partition = out
        self.params_dtype = params_dtype
        weight_packed = _PPModelWeight(
            data=torch.empty(out, input_size_per_partition // 2, dtype=torch.uint8),
            input_dim=1,
            output_dim=0,
            weight_loader=weight_loader,
        )
        layer.register_parameter("weight_packed", weight_packed)
        weight_scale = _PPGroupScale(
            data=torch.empty(
                out, input_size_per_partition // self.group_size, dtype=torch.uint8
            ),
            input_dim=1,
            output_dim=0,
            weight_loader=weight_loader,
    ...
    def _process_marlin(self, layer) -> None:
        ...
        inp = layer.input_size_per_partition

        # weight: uint8[out, in//2] -> int32[out, in//8] -> [in//8, out] -> marlin
        qweight = layer.weight_packed.data.contiguous().view(torch.int32)
        qweight = qweight.t().contiguous()
        perm = torch.empty(0, dtype=torch.int, device=device)
        marlin_q = gptq_marlin_repack(
            b_q_weight=qweight, perm=perm, size_k=inp, size_n=out, num_bits=4
        )

        # scale: uint8 e8m0[out, in//32] -> 2**e -> [in//32, out] -> marlin perm -> e8m0
        scale = _normalize_scale_tensor(layer.weight_scale.data, self.params_dtype)
        scale = scale.t().contiguous()
        marlin_s = marlin_permute_scales(
            scale, size_k=inp, size_n=out, group_size=self.group_size
        )
        marlin_s = mxfp4_marlin_process_scales(marlin_s, input_dtype=self.params_dtype)
        ...
        wtype = scalar_types.float4_e2m1f
        # MXFP4 has no per-tensor global scale (e8m0 group scale is all there is),
        # but the float4_e2m1f Marlin kernel requires a non-empty global_scale;
        # pass 1.0 so the effective scale is exactly the e8m0 group scale.
        gscale = torch.ones(1, dtype=torch.float32, device=device)

        # Validate the GEMM tiling *before* committing: the dense Marlin float4
        # kernel can't find a thread config for group_size=32 at wide N (merged
        # qkv/gate_up) with the sm90 shared-mem budget. If it raises here (with
        # weight_packed/weight_scale still intact), process_weights_after_loading
        # catches it and falls back to dequant for THIS layer only.
        _ = gptq_marlin_gemm(
            torch.zeros(1, inp, dtype=self.params_dtype, device=device),
            None, marlin_q, marlin_s, gscale, zp, g_idx, g_idx_sort, workspace,
            wtype, size_m=1, size_n=out, size_k=inp, is_k_full=True,
        )
        torch.cuda.synchronize(device)

        del layer.weight_packed
        del layer.weight_scale
        layer.weight_packed = _PPParameter(marlin_q, requires_grad=False)
        layer.weight_scale = _PPParameter(marlin_s, requires_grad=False)
        layer.g_idx = g_idx
        layer.g_idx_sort_indices = g_idx_sort
        layer.weight_zp = zp
        layer.workspace = workspace
        layer.wtype = wtype
        layer.global_scale = gscale
        self._mode = "marlin"

    def _process_dequant(self, layer) -> None:
        wp = layer.weight_packed.data
        ws = layer.weight_scale.data
        out = layer.output_size_per_partition
        inp = layer.input_size_per_partition
        vals = _pp_unpack_fp4(wp, out, inp)
        scale = torch.pow(2.0, ws.float() - 127.0).repeat_interleave(
            self.group_size, dim=1
        )
        weight = (vals * scale).to(torch.bfloat16)
        del layer.weight_packed
        del layer.weight_scale
        layer.weight = _PPParameter(weight, requires_grad=False)
        self._mode = "dequant"

    def process_weights_after_loading(self, layer) -> None:
        if self.kernel == "marlin":
            try:
                self._process_marlin(layer)
                logger.info_once("proof-pilot MXFP4: using Marlin fp4 kernel")
                return
            except Exception as e:  # noqa: BLE001
                logger.warning(
                    "proof-pilot MXFP4 Marlin prep failed (%s); "
                    "falling back to dequant-to-bf16",
                    repr(e),
                )
        self._process_dequant(layer)
        logger.info_once("proof-pilot MXFP4: using dequant-to-bf16 path")

    def apply_weights(self, layer, x, bias=None):
        if self._mode == "marlin":
            # apply_gptq_marlin_linear hard-codes global_scale=None, which the
            # float4_e2m1f kernel rejects -> call gptq_marlin_gemm directly with
            # global_scale=1.0.
            from sglang.jit_kernel.gptq_marlin import gptq_marlin_gemm
        ...
        return _pp_F.linear(x, layer.weight, bias)


# =================== end proof-pilot patch (dense MXFP4) ===================
...
class CompressedTensorsConfig(QuantizationConfig):
    ...
    def _get_scheme_from_parts(
        ...
        # proof-pilot: dense MXFP4 / float4 group weight-only -> dequant-to-bf16
        # scheme. Must come before _is_wNa16 (which would route float4 into the
        # int4 branch and raise). NVFP4 uses strategy=tensor_group, not matched.
        if (
            getattr(weight_quant.type, "value", weight_quant.type) == "float"
            and weight_quant.num_bits == 4
            and weight_quant.strategy
            in (QuantizationStrategy.GROUP.value, "group")
        ):
            return CompressedTensorsW4A16Fp4Dense(
