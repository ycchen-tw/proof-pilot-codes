    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if name.endswith(".bias") and name not in params_dict:
                    continue
                # Remap compressed-tensors fp8 KV-cache scales
                # (self_attn.{k,v}_scale -> self_attn.attn.{k,v}_scale). Returns
                # None (-> skip) when the RadixAttention has no such scale param
                # (e.g. bf16 KV), so a KV-scale checkpoint still loads either way.
                # Upstream olmo2.py lacks this; standard pattern in llama/qwen.
