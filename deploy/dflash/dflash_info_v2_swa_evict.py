...
class DFlashDraftInputV2(SpecInput):
    ...
    def prepare_for_decode(self, batch: ScheduleBatch):
        ...
        if bs == 0:
            return
        # proof-pilot: free out-of-window SWA KV every decode step (mirror EAGLE's
        # prepare_for_decode). Stock DFLASH never calls this, so the SWA pool fills with
        # ~(total-window) tokens per req and the 1/10-sized swa pool retracts at ~20k under
        # concurrency. Gated internally by req.decode_batch_idx % eviction_interval (the
        # interval = sliding_window * SGLANG_SWA_EVICTION_INTERVAL_MULTIPLIER); we tick
        # decode_batch_idx in the per-req loop below. No-op when the model isn't hybrid-SWA.
        ...
        for i, req in enumerate(batch.reqs):
            req.decode_batch_idx += 1  # proof-pilot: tick for maybe_evict_swa() gate
